...
def table(
    doc: Document, headers: list[str], rows: list[list[str]], widths: list[float] | None = None
) -> None:
...
TEAMMATES = [
    {
        ...
        "task_files": [
            ...
            ("app/templates/qualifications/list.html", "the table headings the user clicks"),
            (
    ...
    },
    {
        ...
        "task_files": [
            ...
            ("app/services/audit_service.py", "the layer the page talks to"),
            ("app/routes/admin_routes.py", "the audit() view and a new export view"),
            ("app/templates/admin/audit.html", "the filter form and the download button"),
            ("tests/integration/test_workflows.py", "where you add tests"),
        ],
        "task_hint": (
    ...
    },
    {
        ...
        "task_files": [
            ...
            ("app/config.py", "where the five attempts and fifteen minutes are configured"),
            (
                "app/services/user_service.py",
                "the authenticate() function - this is where the locking decision goes",
            ),
            ("tests/unit/test_auth_and_audit.py", "where you add tests"),
        ],
        "task_hint": (
...
def why_section(doc: Document) -> None:
    ...
    table(
        ...
        [
            ["Repository (or repo)", "The shared folder holding all our code and documents."],
            [
                "Branch",
...
def setup_section(doc: Document, person: dict) -> None:
    ...
    if person["needs_account"]:
        ...
        callout(
            ...
            "GitHub sends a confirmation link and you cannot continue without " "clicking it.",
        )
    ...
    table(
        ...
        [
            [
                "The 'Code' tab",
                "All the files. The README on the front page explains the whole " "project.",
            ],
            ["The 'Issues' tab", "The list of jobs to be done. Yours will go here."],
            ["The 'Pull requests' tab", "Changes waiting to be reviewed and included."],
            ["The 'Actions' tab", "The automatic checking. Green tick means everything passed."],
            [LIVE_URL, "The actual working system. Ask " + OWNER + " for a login."],
        ],
        [1.9, 4.3],
    )
...
def browser_workflow(doc: Document, person: dict) -> None:
    ...
    step(
        doc,
        3,
        "You will be offered some templates. Choose 'Assigned task' and click " "'Get started'.",
    )
    step(doc, 4, "Fill in the title and the boxes, then click 'Submit new issue'.")
    step(doc, 5, "Note the number it is given - something like #7. You will need it later.")
    ...
    step(
        doc,
        2,
        "Click through the folders to the file you want. Folders are listed " "first, then files.",
    )
    step(
        doc,
        3,
    ...
    step(
        doc, 2, "Click the 'Files changed' tab. Green lines were added, red lines were " "removed."
    )
    step(
        doc,
        3,
...
def practice_section(doc: Document, person: dict) -> None:
    ...
    step(
        doc,
        4,
        "Press Ctrl+F and search for 'INSERT NAME' to find the team table near " "the bottom.",
    )
    step(
        doc,
        5,
    ...
    command(
        doc, f"| `[INSERT NAME]` `[INSERT STUDENT NUMBER]` | {AREA_LOOKUP[person['student']]} |"
    )
    ...
    command(
        doc, "Fills in my row of the team table, which still held a\n" "placeholder.\n\nCloses #7"
    )
...
def task_section(doc: Document, person: dict) -> None:
    ...
    bullet(doc, "Read the error message. It usually says exactly what is wrong.", "Start here: ")
    bullet(
        doc,
        "Look at how something similar was already done in the same file. "
        "Almost everything you need already has an example.",
    )
    bullet(
        doc,
        "Make a smaller change. Get one small piece working, commit it, " "then do the next piece.",
    )
    bullet(doc, f"Ask {OWNER}. Send him the link to your pull request and the red " "error text.")
    bullet(
        doc,
        "Comment on your own pull request describing what you tried. That "
...
def commands_section(doc: Document, person: dict) -> None:
    ...
    table(
        ...
        ["Problem", "Command"],
        [
            ["I want to throw away my changes to one file", "git restore path/to/file.py"],
            ...
            ["My branch is behind develop", "git merge origin/develop"],
        ],
        [2.9, 3.3],
    )
...
def evidence_section(doc: Document, person: dict) -> None:
    ...
    para(
        doc, "There is a template waiting for you in the project. Open this file and " "fill it in:"
    )
...
def troubleshooting_section(doc: Document) -> None:
    ...
    table(
        ...
        [
            ...
            [
                ...
                "Nothing is lost. Tell " + OWNER + " - it takes him a minute to " "move it.",
            ],
            [
                "I cannot find my pull request",
                REPO_URL + "/pulls - then click 'Closed' if it has been merged " "already.",
            ],
        ],
        [1.9, 4.3],
    )
